...
DWORD KinectFusionProcessor::MainLoop()
{
	...
	//init some stuff for saving images and the camera matrix
	fileSuffix = 0;
	std::remove("cameraMatrix.txt");
...
void KinectFusionProcessor::ProcessDepth()
{
    ...
    else
    {
		...
		camFile.open("cameraMatrix.txt", std::ios::out | std::ios::app);
		camFile << calculatedCameraPose.M11 << " " << calculatedCameraPose.M12 << " " << calculatedCameraPose.M13 << " " << calculatedCameraPose.M14 << " "
...
/// </returns>
HRESULT KinectFusionProcessor::GetScreenshotFileName(wchar_t *screenshotName, UINT screenshotNameSize)
{
...
void KinectFusionProcessor::ProcessColor()
{
	HRESULT hr;
	NUI_IMAGE_FRAME imageFrame;
